...
func UpdatecreditsessionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateUpdatecreditsessionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_payments_v1_sessions_session_id",
		mcp.WithDescription("Update an existing payment session"),
		mcp.WithString("session_id", mcp.Required(), mcp.Description("session_id")),
		mcp.WithString("purchase_country", mcp.Description("Input parameter: The purchase country of the customer. The billing country always overrides purchase country if the values are different. Formatted according to ISO 3166 alpha-2 standard, e.g. GB, SE, DE, US, etc.")),
		...
		mcp.WithString("purchase_currency", mcp.Description("Input parameter: The purchase currency of the order. Formatted according to ISO 4217 standard, e.g. USD, EUR, SEK, GBP, etc.")),
		...
		mcp.WithNumber("order_amount", mcp.Description("Input parameter: Total amount of the order including tax and any available discounts. The value should be in non-negative minor units. Eg: 25 Euros should be 2500.")),
		mcp.WithArray("order_lines", mcp.Description("Input parameter: The array containing list of line items that are part of this order. Maximum of 1000 line items could be processed in a single order.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    UpdatecreditsessionHandler(cfg),
	}
}
